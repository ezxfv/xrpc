...
func (p *consulRegisterPlugin) Connect(conn net.Conn) (net.Conn, bool) {
	if p.Metrics != nil {
		clientMeter := metrics.GetOrRegisterMeter("clientMeter", p.Metrics)
		clientMeter.Mark(1)
	}
...
func (p *consulRegisterPlugin) RegisterService(sd *xrpc.ServiceDesc, ss interface{}) (err error) {
	name := sd.ServiceName
