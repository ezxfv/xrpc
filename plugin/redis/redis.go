...
func New() *redisRegisterPlugin {
	return &redisRegisterPlugin{
		ServiceAddress: "",
		RedisServers:   nil,
		BasePath:       "",
		Metrics:        nil,
...
type redisRegisterPlugin struct {
	...
	// redis addresses
	RedisServers []string
	// base path for xrpc server, for example com/example/xrpc
	BasePath string
...
func (p *redisRegisterPlugin) Start() error {
	...
	err := p.kv.Put(p.BasePath, []byte("xrpc_path"), &store.WriteOptions{IsDir: true})
...
func (p *redisRegisterPlugin) Connect(conn net.Conn) (net.Conn, bool) {
	if p.Metrics != nil {
		clientMeter := metrics.GetOrRegisterMeter("clientMeter", p.Metrics)
		clientMeter.Mark(1)
	}
...
func (p *redisRegisterPlugin) RegisterService(sd *xrpc.ServiceDesc, ss interface{}) (err error) {
	name := sd.ServiceName
	metadata := sd.Metadata.(string)
